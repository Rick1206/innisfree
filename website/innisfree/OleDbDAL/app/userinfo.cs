...
namespace Maticsoft.OleDbDAL.app
{
	/// <summary>
	/// 数据访问类:userinfo
	/// </summary>
	public partial class userinfo:Iuserinfo
	{
		...
		public bool Add(Maticsoft.Model.app.userinfo model)
		{
			...
			OleDbParameter[] parameters = {
					...
					new OleDbParameter("@usex", OleDbType.Boolean,1),
					new OleDbParameter("@uage", OleDbType.Integer,4),
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.app.userinfo DataRowToModel(DataRow row)
		{
			Maticsoft.Model.app.userinfo model=new Maticsoft.Model.app.userinfo();
			if (row != null)
			{
				...
				if(row["uage"]!=null && row["uage"].ToString()!="")
				{
					model.uage=int.Parse(row["uage"].ToString());
				}
				if(row["uphone"]!=null)
				{
